...
export const createStudent = handleAsync(async (req, res) => {
  const student = await studentRepo.createStudent(req.body);
  res.status(201).json(student);
});

export const generateAdmissionAndCreateStudent = handleAsync(async (req, res) => {
...
export const getAllStudents = handleAsync(async (_req, res) => {
  const students = await studentRepo.findAllStudents();
  res.json(students);
});

export const getStudentWithGuardianById = handleAsync(async (req, res) => {
  const student = await studentRepo.findStudentWithGuardianById(req.params.id);
  if (!student) throw new AppError("Student not found", 404);
  res.json(student);
});

export const getStudentNameById = handleAsync(async (req, res) => {
...
export const getStudentsByClassName = handleAsync(async (req, res) => {
...
export const updateStudentById = handleAsync(async (req, res) => {
...
export const deleteStudentById = handleAsync(async (req, res) => {
...
export const deleteAllStudents = handleAsync(async (_req, res) => {
  await studentRepo.deleteAllStudents();
  res.status(204).send();
});

export const countStudents = handleAsync(async (req, res) => {
  const count = await studentRepo.countStudents(req.params.id);
  if (count === null) throw new AppError("Invalid student ID", 400);
  res.json({ count });
});

export const getAllStudentNames = handleAsync(async (_req, res) => {
